...
with st.spinner(text = "Fetching batch of inference data"):
    ...
    st.sidebar.write('✅ Batch of inference data was fetched ')
    progress_bar.progress(2/N_STEPS)
    print(f'{features}')

with st.spinner(text = "Loading ML model from the registry"):
    model = load_model_from_registry()
    st.sidebar.write('✅ ML model was loaded ')
    progress_bar.progress(3/N_STEPS)

with st.spinner(text = "Predicting demand"):
    predictions = get_model_predictions(model, features)
    st.sidebar.write('✅ Predictions were made ')
